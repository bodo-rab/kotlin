        ...
        for (supertypeEntry in declaration.superTypeListEntries) {
            if (supertypeEntry is KtDelegatedSuperTypeEntry) {
                ...
                if (!DescriptorUtils.isInterface(typeDescriptor)) {
                    ...
                    return
                } else if (typeDescriptor.getAllSuperClassifiers().any {
                        it.fqNameUnsafe == StandardNames.FqNames.cloneable ||
                                it.fqNameUnsafe == javaLangCloneable
                    }
                ) {
                    trace.report(Errors.VALUE_CLASS_CANNOT_BE_CLONEABLE.on(typeReference))
                    return
                }
            }
        }
